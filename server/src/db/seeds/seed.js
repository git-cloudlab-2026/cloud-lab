...
async function seed() {
  await withTransaction(async (client) => {
    ...
    await client.query(`
      ...
      (1, 3, 1, NULL, 'request_created', 'success', 'Amir a cree une demande Linux Admin.'),
      (2, 1, 1, NULL, 'request_approved', 'success', 'Nadia a approuve la demande.'),
      (3, NULL, 1, 1, 'vm_created', 'success', 'La VM git-linux-admin-amir-001 a ete creee.'),
      (4, NULL, 1, 1, 'ansible_completed', 'success', 'Le playbook Linux Admin est termine.'),
      (5, NULL, 5, 2, 'vm_expired', 'warning', 'La VM git-cyber-amir-001 est arrivee a expiration.')
    `);

    for (const table of ["users", "courses", "vm_templates", "vm_requests", "virtual_machines", "vm_metrics", "cost_records", "audit_events"]) {
...
seed()
  .then(() => {
    console.log("Seed PostgreSQL termine.");
  })
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
