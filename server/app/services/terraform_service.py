...
class TerraformService:
    ...
    async def create_vms(self, request: VmRequest, template: VmTemplate, owner: User) -> list[TerraformVm]:
        ...
        tfvars = {
            ...
            "allowed_node_exporter_cidrs": settings.terraform_allowed_node_exporter_cidrs,
    ...
    def _prepare_run_dir(self, module_dir: Path, run_dir: Path) -> None:
        ...
        for source in module_dir.iterdir():
            if source.name in {".terraform", "terraform.tfstate", "terraform.tfstate.backup"}:
                continue
    ...
    def _ssh_public_key(self) -> str:
        settings = get_settings()
        if settings.terraform_ssh_public_key:
            return settings.terraform_ssh_public_key.strip()
        if settings.terraform_ssh_public_key_path:
            path = Path(settings.terraform_ssh_public_key_path).expanduser()
            if path.exists():
                return path.read_text(encoding="utf-8").strip()
        raise ApiError(
            500,
            "terraform_missing_ssh_key",
            "Configurer TERRAFORM_SSH_PUBLIC_KEY ou TERRAFORM_SSH_PUBLIC_KEY_PATH.",
        )
